...
function setupFormHandlers() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            if (validateAcademicForm(this)) {
                showNotification('Thank you for your inquiry! Our academic office will respond within 48 hours.', 'success');
                this.reset();
            }
        });
    }
    ...
    document.querySelectorAll('.contact-item').forEach(item => {
        item.addEventListener('click', function() {
            ...
            if (email && email.includes('@')) {
                window.location.href = `mailto:${email.split('\\n')[0]}`;
            }
        });
    });
}
...
function setupAnalytics() {
    ...
    const trackingEvents = [
        ...
        { selector: '.contact-item', event: 'click', category: 'Contact' }
    ];
...
function openImageModal(imageSrc) {
    event.stopPropagation(); // Prevent the link from opening
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (imageModal) {
        ...
        console.warn('imageModal not found in DOM');
    }
});
